  ...
  if (hereengelle == "acik") {
    ...
    if (here.some(word => msg.content.toLowerCase().includes(word))) {
      if (!msg.member.hasPermission("ADMINISTRATOR")) {
        ...
        var revers2 = new Discord.MessageEmbed()
          .setColor("BLACK")
          .setDescription(`Bu Sunucuda Everyone ve Here Yasak!`);
        msg.channel.send(revers2);
      }
    }
...
client.on("guildMemberAdd", async member => {
  ...
  const arrow = client.emojis.cache.get("722640275993788523")
  const verifyme= client.emojis.cache.get("722641748005879828")
  const revers3 = new Discord.MessageEmbed()
    ...
    .setDescription(`<@${member.user.id}>**, Aramıza Hoşgeldin.**
     ...
     ${arrow} **Teyit Olabilmeniz İçin Ses Teyit Odalarından Birine Geçiniz.**
     ${arrow} **Kayıt Tarihi: ${moment.utc(member.user.createdAt).format('DD.MM.YY')}**
     ${arrow} **Güvenli Hesap!**

    ${verifyme} **Bol keyifli zaman geçirmeniz dileğiyle - Trigger.**
    
  

`,true)
    .setTimestamp()
    .setImage("https://cdn.discordapp.com/attachments/522576410083328010/838797746969772083/r4l.gif")
...
client.on("message", async message => {
  ...
  if (REASON) {
    ...
    const reversafk = new Discord.MessageEmbed()

      .setColor("RANDOM")
      .setDescription(`**Bu Kullanıcı Afk**\n\n**Afk Olan Kullanıcı :** \`${USER.tag}\`\n**Afk Süresi :** \`${timeObj.hours}saat\` \`${timeObj.minutes}dakika\` \`${timeObj.seconds}saniye\`\n**Sebep :** \`${REASON}\``);

    message.channel.send(reversafk);
  }
});
...
client.on("message", async message => {
  const revers = message.content.toLocaleLowerCase();

  if (
    revers === "selam" ||
    revers === "sa" ||
    revers === "selamün aleyküm" ||
    revers === "selamun aleyküm" ||
    revers === "slm" ||
    revers === "sea"
  ) {
    ...
    return message.channel.send(revers5)
    }
  }
});
...
client.on('channelCreate', async channel => {
  const c = channel.guild.channels.cache.get(db.fetch(`reversmodlog_${channel.guild.id}`));
  if (!c) return;
...
client.on('emojiCreate', emoji => {
  const c = emoji.guild.channels.cache.get(db.fetch(`reversmodlog_${emoji.guild.id}`));
  if (!c) return;
...
client.on('emojiUpdate', (oldEmoji, newEmoji) => {
  const c = newEmoji.guild.channels.cache.get(db.fetch(`reversmodlog_${newEmoji.guild.id}`));
  if (!c) return;
...
client.on('guildBanAdd', async (guild, user) => {    
    const channel = guild.channels.cache.get(db.fetch(`reversmodlog_${guild.id}`));
  if (!channel) return;
...
client.on('messageDelete', async message => {    
  if(message.author.bot) return
    ...
    const channel = message.guild.channels.cache.get(db.fetch(`reversmodlog_${message.guild.id}`));
  if (!channel) return;
...
client.on('messageUpdate', async(oldMessage, newMessage) => {
    ...
    const channel = oldMessage.guild.channels.cache.get(db.fetch(`reversmodlog_${oldMessage.guild.id}`));
    if(!channel) return;
...
client.on('roleCreate', async (role) => {    
    ...
    const channel = role.guild.channels.cache.get(db.fetch(`reversmodlog_${role.guild.id}`));
  if (!channel) return;
...
client.on('voiceStateUpdate', (oldMember, newMember) => {
 ...
 // if (!logA[oldMember.guild.id]) return;
  ...
  if (db.has(`reversmodlog_${oldMember.guild.id}`) === false) return;
  
  var kanal = oldMember.guild.channels.cache.get(db.fetch(`reversmodlog_${oldMember.guild.id}`).replace("<#", "").replace(">", ""))
  if (!kanal) return;
